...
app.post('/confirm-order', csrfProtection, async (req, res) => {
  try {
    const { orderId, paymentIntentId, razorpay_payment_id, razorpay_signature } = req.body;
    
    // Input validation
    if (!orderId || !paymentIntentId || !razorpay_payment_id || !razorpay_signature) {
      return res.status(400).json({ 
        error: 'Missing required fields',
        required: ['orderId', 'paymentIntentId', 'razorpay_payment_id', 'razorpay_signature']
      });
    }
    ...
    const generated_signature = crypto
      ...
      .update(paymentIntentId + '|' + razorpay_payment_id)
      .digest('hex');
